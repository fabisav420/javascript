...
impl ContextifyScript {
  ...
  pub fn new<'s>(
    ...
    let context = if let Some(parsing_context) = parsing_context {
      let Some(context) =
        ...
        return Err(type_error("Invalid sandbox object"));
      };
    ...
    let source = if let Some(cached_data) = cached_data {
    ...
    if produce_cached_data {
      // let cached_data = unbound_script.create_code_cache();
    }

    Ok(Self {
      script: v8::TracedReference::new(scope, unbound_script),
    })
  }
  ...
  fn run_in_context<'s>(
    ...
    let context = if let Some(sandbox) = sandbox {
      let context = ContextifyContext::from_sandbox_obj(scope, sandbox)
    ...
    let result = self
      .eval_machine(scope, context, timeout, display_errors, break_on_sigint)
  ...
  pub fn eval_machine<'s>(
    ...
    if timed_out.load(Ordering::Relaxed) {
      ...
      let message = v8::String::new(scope, "Timed out").unwrap();
...
impl ContextifyContext {
  pub fn attach(
    ...
    _name: String,
    _origin: String,
    _allow_code_gen_strings: bool,
    _allow_code_gen_wasm: bool,
    _own_microtask_queue: bool,
  ) {
...
pub fn op_vm_create_script<'a>(
  ...
  let script = ContextifyScript::new(
    scope,
    source,
    filename,
    line_offset,
...
pub fn op_vm_compile_function<'s>(
  ...
  if produce_cached_data {
    // let cached_data = function.create_code_cache();
  }
